...
function processContains(item, list, callback) {
  /* CODE HERE ASK HOW TO DO THIS PROBLEM WITH ONE OF THE 4 methods we covered */
  return callback(list.includes(item));
 // ASK HOW YOU CAN DO THIS WITH ONE OF THE FOUR METHODS
};
...
function getFullNames(runners) {
  ...
  return fullName.push(`${el.last_name}, ${el.first_name}`)
  })
  return fullName;
}
...
function firstNamesAllCaps(runners) {
  /* CODE HERE */
  const upperNames = runners.map(function(current_val){
    return current_val.first_name.toUpperCase();
  })
  return upperNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  /* CODE HERE */
  const sizeArray = runners.filter(function(current_val){
    return current_val.shirt_size === tShirtSize;
  })
  return sizeArray;
}
...
function tallyUpDonations(runners) {
  /* CODE HERE */
  const total = runners.reduce(function(accumulator,current_val){
    return accumulator + current_val.donation;
  },0)
  return total;
}
 ...
 * etc
*/
function counterMakerWithLimit(a) {
  /* CODE HERE */
  var count = 0;
